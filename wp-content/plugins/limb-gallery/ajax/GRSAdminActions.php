...
class GRSAdminActions extends GRSGalleryAjax {
	...
	private function getItems( $directories = array(), $namee = '', $parentDirIndex = - 1 ) {
		...
		foreach ( $directories as $key => $name ) {
			...
			if ( is_dir( $uploadDir . $files->path . $name . '/' ) ) {
				...
				$this->getItems( scandir( $uploadDir . $files->path . $name . '/' ), $files->path . $name,
					$files->curentDirIndex );
			} else {
	...
	private function delItemsFromUploader() {
		...
		if ( is_array( $resArr ) ) {
			...
			$this->result( 'error', __( 'Something went wrong', 'limb-gallery' ) );
		}
	}
	...
	private function getGrsGallerieContent( $galId ) {
		global $wpdb;
		$query             = $wpdb->prepare( "SELECT * FROM `" . $wpdb->prefix . "limb_gallery_galleriescontent` WHERE galId=%d ORDER BY `order` ASC",
			$galId );
	...
	private function saveOrder() {
		...
		if ( $id && is_array( $items ) ) {
			...
			$this->result( 'error', __( 'There is no items selected or', 'limb-gallery' ) );
		}
		if ( $success !== false ) {
			$this->result( 'success', array() );
		} else {
			$this->result( 'error', __( 'Problem with Order saving', 'limb-gallery' ) );
		}
	}
	...
	private function insert() {
		...
		if ( $success !== false ) {
			...
			$this->result( 'error', __( 'Problem with gallery save', 'limb-gallery' ) );
		}
	}
	...
	private function update() {
		...
		if ( $successGal !== false && $successImg !== false ) {
			...
			$this->result( 'error', __( 'Problem with image or gallery update', 'limb-gallery' ) );
		}
	}
	...
	private function addImages() {
		...
		foreach ( $files as $key => $file ) {
			...
			if ( function_exists( 'getimagesize' ) ) {
				...
				$this->result( 'error',
					__( 'Function not found: getimagesize. Please connect to plugin support.', 'limb-gallery' ) );
	...
	private function deleteImage() {
		...
		$success = $wpdb->delete( $wpdb->prefix . 'limb_gallery_galleriescontent', array( 'id' => $id ),
			array( '%d' ) );
		//$success (count of deleted rows)
		if ( $success !== false ) {
			$this->result( 'success', $success );
		} else {
			$this->result( 'error', __( 'Problem with delete', 'limb-gallery' ) );
		}
	}
	...
	private function removeComments() {
		...
		if ( $id ) {
			...
			$this->result( 'error', __( 'Invalid comment id', 'limb-gallery' ) );
		}
	}
	...
	private function saveSettings() {
		...
		$config       = LIMB_Gallery::$config;
		$save         = $wpdb->update( $wpdb->prefix . "limb_gallery_settings",
			array(
				'timezone'          => ( $config->comments ? sanitize_text_field( $settings['timezone'] ) : 'UTC' ),
				'timeformat'        => ( $config->comments ? sanitize_text_field( stripslashes( $settings['timeformat'] ) ) : 'F d y h:i:s' ),
				'collapseNavClicks' => (int) ( $config->effects ? sanitize_text_field( $settings['collapseNavClicks'] ) : 1 ),
				'clicksCount'       => (int) ( $config->effects ? sanitize_text_field( $settings['clicksCount'] ) : 2 ),
				'hideNavButton'     => (int) ( $config->effects ? sanitize_text_field( $settings['hideNavButton'] ) : 0 ),
				'showVmTitle'       => (int) $settings['showVmTitle'],
				'showYtTitle'       => (int) $settings['showYtTitle'],
				'closeLbOnSide'     => (int) ( $config->effects ? sanitize_text_field( $settings['closeLbOnSide'] ) : 1 ),
				'openCommTrig'      => (int) ( $config->comments ? sanitize_text_field( $settings['openCommTrig'] ) : 0 ),
				'showTitleDescTrig' => (int) $settings['showTitleDescTrig'],
				'fmImMoveCount'     => (int) ( $config->filmstrip ? sanitize_text_field( $settings['fmImMoveCount'] ) : 2 ),
				'filmImMoveCount'   => (int) ( $config->filmstrip ? sanitize_text_field( $settings['filmImMoveCount'] ) : 2 ),
	...
	private function uninstall() {
		...
		if ( $result !== false ) {
			...
			$this->result( 'error', __( 'Problem with uninstall', 'limb-gallery' ) );
		}
...
//die();
	...
	private function saveThemeToFile( $id ) {
		global $wpdb;
		$grsTheme = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM `" . $wpdb->prefix . "limb_gallery_themes` WHERE `id` = %d",
			$id ) );
